...
import { getFollowedChannels, getChannels as getChannelsRequest } from "../../api/api";
...
const  useChannels = () => {   
    ...
    const getChannels = async (isLogged = false) => {
        ...
        if(followedChannelsData.error){
            return toast.error(followedChannelsData.exception?.response?.data || "Error occured when fetching the followed  channels")
        }
